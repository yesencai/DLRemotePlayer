...
@interface DLResourceLoader ()<DLAudioDownLoaderDelegate>

@property (nonatomic, strong) DLAudioDownLoader *downLoader;
...
- (DLAudioDownLoader *)downLoader {
    if(!_downLoader) {
        _downLoader = [[DLAudioDownLoader alloc] init];
...
- (void)handleAllRequest {
    ...
    for (AVAssetResourceLoadingRequest *loadingRequest in self.loadingRequests) {
        ...
        NSData *data = [NSData dataWithContentsOfFile:[DLAudioFileTool tmpPathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        if (data.length == 0) {
            data = [NSData dataWithContentsOfFile:[DLAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        }
...
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader shouldWaitForLoadingOfRequestedResource:(AVAssetResourceLoadingRequest *)loadingRequest
{
    ...
    if ([DLAudioFileTool isCacheFileExists:url]){
        // 三个步骤, 直接响应数据
...
- (void)handleRequestWithLoadingRequest: (AVAssetResourceLoadingRequest *)loadingRequest {
    NSURL *url = [loadingRequest.request.URL httpURL];
    // 1. 填充信息头
    loadingRequest.contentInformationRequest.contentType = [DLAudioFileTool contentTypeWithURL:url];
    loadingRequest.contentInformationRequest.contentLength = [DLAudioFileTool cacheFileSizeWithURL:url];
    loadingRequest.contentInformationRequest.byteRangeAccessSupported = YES;
    
    
    // 2. 响应数据
    NSData *data = [NSData dataWithContentsOfFile:[DLAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
